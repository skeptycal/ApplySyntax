...
class PackageSearch(object):
    ...
    # Search All
    ################
    def find_raw(self, pattern, regex=False, st_search=False):
        """
    ...
    # Search Override
    ################
    def find(self, pattern, regex, st_search=False):
        """
    ...
    def search(self, **kwargs):
        ...
        return self.find(pattern, regex) if not self.find_all else self.find_raw(pattern, regex)

    def sublime_search(self, **kwargs):
        """
        Search packages with quick panel
        """
        kwargs = self.pre_process(**kwargs)
        pattern = kwargs.get("pattern", None)
        regex = kwargs.get("regex", False)
        ...
        if not self.find_all:
            self.find(pattern, regex, st_search=True)
        else:
            self.find_raw(pattern, regex, st_search=True)
